    ...
    def check_connection(self):
        ...
        try:
            response = s3_client.get_object(
                Bucket=self.test_bucket_name, Key=self.test_file
            )
            file_content = response["Body"].read().decode("utf-8")
            if file_content != "I am health!":
                raise ValueError(
                    "Содержимое не соответствует ожидаемой строке 'I am health!'"
                )
            else:
...
if __name__ == "__main__":
    ...
    aws_user = AWSUser(access_key=access_key, secret_key=secret_key)

    aws_user.reading_file_to_s3("testloaduser", "test.txt")

    aws_user.upload_file_to_s3(
        "mercedesbucket", "available_products.xls", "../dev/available_products.xls"
    )
    aws_user.upload_file_to_s3(
        "mercedesbucket",
